...
2. **天氣和新聞查詢（使用 Ollama）** - 加入工具使用能力，零成本學習
3. **RAG 入門體驗** - 認識 RAG 與文件問答；完整 RAG 由淺至深請見 [整合 RAG 的實作](./RAG/README.md)
...
### [整合 RAG 的實作](./RAG/README.md)

RAG（檢索增強生成）是 AI 應用的重要功能，讓模型能依據**你的文件**回答問題。本區為獨立大章節，範例由淺至深：入門（記憶體儲存、精簡版問答）→ 基礎（向量持久化、多來源）→ 進階（本地／雲端向量資料庫）。完整「RAG 範例大綱」與各範例連結請見 [RAG/README.md](./RAG/README.md)。
