...
- [⭐⭐⭐ 範例七：動態文件生成與 PDF 自動化](#-範例七動態文件生成與-pdf-自動化)
- [🎯 學習路徑建議](#-學習路徑建議)
...
### [⭐⭐⭐ 範例七：動態文件生成與 PDF 自動化](./動態文件生成與PDF自動化/README.md)
**難度**: 進階 | **學習時間**: 45-60 分鐘

使用 Google Docs 範本與 `{{placeholder}}` 佔位符替換技術，根據申請資料動態產生客製化結業證書/合約，自動匯出為 PDF 檔案並透過 Gmail 自動寄出附件。

**學習重點**：
- Google Docs 範本設計與 `{{placeholder}}` 佔位符規則
- Google Drive 節點（檔案複製 Copy 與 PDF 匯出 Download）
...
7. **動態文件生成與 PDF 自動化** → 掌握 Docs 範本佔位符替換、PDF 轉存與郵件附件自動化流程
