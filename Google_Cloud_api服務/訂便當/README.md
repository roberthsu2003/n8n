# 整合 Google 服務
## 範例 4：辦公室線上訂便當與 Google Sheets 自動歸檔

### 📚 工作流程說明

這個 n8n 工作流程建立了一個完整的線上訂便當系統。同仁可以透過網頁表單選擇主餐與填寫姓名，系統會自動將訂單資料整理後逐筆儲存到 Google 試算表中，大幅提升福委會與總務統計訂單的效率。
...
```mermaid
flowchart LR
    A["📝 網頁填表 (Form Trigger)"] --> B["✂️ 拆解主餐與價格 (Code / Set 節點)"]
    B --> C["📊 Google Sheets (Append Row 寫入試算表)"]
    C --> D["🖥️ 網頁顯示訂購成功確認畫面"]
```
