...
> 💡 **任務目標**：讓 AI 建立包含會員資料與多筆訂單的巢狀 JSON，並利用表達式自動運算訂單統計。

**可直接複製給 AI 的 Prompt 提詞**：
```text
請協助我在 n8n 建立一個「進階 JSON 巢狀資料處理」工作流：
1. 起點使用 Code 節點或 Set 節點，生成一組包含訂單資訊的 JSON 物件（包含 customer_name: "王小明"、is_vip: true、items 陣列：包含多筆商品名稱與價格）。
2. 新增一個 Set 節點，使用 n8n 表達式提取出：
   - 購買總品項數量 (items.length)
   - 第一個購買的商品名稱 (items[0].name)
   - 判斷是否為 VIP 並回傳折扣文字（如「享 VIP 9 折優惠」）
3. 請配置好各節點與表達式語法，並說明資料如何流動。
```
</details>
