...
| 3 | 基礎 | RAG 基礎：向量持久化與多來源 | 分離式架構（索引／查詢）、Supabase、本機＋Google Drive、來源過濾 | [02 基礎版（Supabase 雲端儲存）](./02_基礎版_Supabase雲端儲存/README.md) |
| 4 | 基礎 | RAG 與檢索策略 | 進階檢索（Hybrid、Rerank）、Chunk 策略、Metadata 過濾 | [04 檢索策略（大綱）](./04_檢索策略/README.md) |
| 5 | 進階 | RAG 進階：本地向量資料庫 | PostgreSQL + PGVector、自建向量庫、完全本地控制 | [03 進階版（本地 PostgreSQL）](./03_進階版_本地PostgreSQL/README.md) |
...
- **入門**：[RAG 智能問答系統（入門版）](./01_入門版_記憶體儲存/README.md)｜[精簡版文件問答（選讀）](../文件智能問答系統/README.md)
