...
## 七、🎯 測試題庫（含工作流程預設題 + 5 道防幻覺驗證題）

### 🌟 工作流程預設測試題（出廠預載・開箱即測）

匯入工作流程後，在「模擬顧客提問」節點中**預設填入的複合型問題**如下：
...
- 💬 **預設提問（`user_question`）**：
  ...
  ```
- 🎯 **標準答案與規章依據拆解**：
  本題為**雙重複合問題**，AI 必須同時檢索並回答兩項政策規定：
  1. **針對「進水是否免費保固」**：
     - **答案**：**不提供免費保固維修**。
     - **依據（第二條 人為損壞與除外條款）**：液體滲入損壞（因進水、受潮、浸泡液體、飲料潑灑等）屬於人為損壞除外條款，不在免費保固範圍內。本公司將依檢測情況酌收零件工本費與**基本檢測費 500 元**。
  2. **針對「退貨需要多久時間」**：
     - **答案**：線上訂購享有 **7 天猶豫期（鑑賞期）**；退貨包裹寄回驗收與退款時程如下：
     - **依據（第三條與第四條 猶豫期與退款時程）**：
       - **驗收時程**：售後中心收到退回包裹後，於 **1 個工作天內** 完成商品驗收。
       - **款項返還**：信用卡付款於驗收無誤後 **3 個工作天內** 完成線上刷退；ATM / 貨到付款則於 **5 個工作天內** 匯入指定銀行帳戶。
- 🔍 **防幻覺檢驗要點**：
  - AI 必須明確回答「進水不享免費保固（需收費）」且能指出「7 天退貨期 / 驗收 1 天 / 信用卡刷退 3 天 / 匯款 5 天」。
  - 兩個子問題皆有憑有據，不能遺漏任何一個。

---

### 🧪 延伸 5 道防幻覺驗證測試題（可替換至「模擬顧客提問」節點測試）

為了進一步驗證 RAG 問答鏈在各種情境下是否能**完全依據規章精準作答、徹底杜絕幻覺**，請將以下問題依序替換至「模擬顧客提問」節點的 `user_question` 欄位進行測試：
