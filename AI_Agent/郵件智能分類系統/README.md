# 整合 LLM 模型的 AI Agent
## 範例 6：Gmail 客服郵件智慧分類與自動歸檔系統

### 📚 工作流程說明

這個範例展示如何建立企業級的客服郵件智能分類與自動化工單系統：
- 接收進線客戶 Email 或表單請求
- 使用 AI Agent 分析郵件類別（投訴/詢價/技術支援/合作）、急迫度與 30 字重點摘要
- 透過 IF 條件節點分流：若為「緊急投訴」，立即發送主管告警通知
- 將所有分析結果與處理建議自動追加寫入 Google 試算表歸檔
...
```mermaid
flowchart TD
    A["📨 接收客戶進線 Email (Webhook / Gmail Trigger)"] --> B["🤖 AI Agent (語義與急迫度分類)"]
    B --> C["⚙️ 解析組裝結構化 JSON (Code 節點)"]
    C --> D{"🚨 是否為高急迫度？"}
    
    D -->|"✅ 高急迫 (投訴/故障)"| E1["⚡ 立即推播緊急主管警報"]
    D -->|"一般諮詢"| E2["📝 標記一般處理工單"]
    
    E1 --> F["📊 Google Sheets (自動追加工單紀錄)"]
    E2 --> F
```
...
- [📥 郵件智能分類系統工作流程樣版 (email_classifier_workflow.json)](./email_classifier_workflow.json)
