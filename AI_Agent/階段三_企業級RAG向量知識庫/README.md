...
歡迎來到 **RAG（檢索增強生成）向量知識庫教學**！這是一個循序漸進的學習系列，從最簡單的記憶體儲存到企業級的雲端與地端向量資料庫，帶您完整掌握 RAG 技術與文件問答系統。
...
| 4 | 進階 | RAG 進階：雲端向量資料庫（Pinecone） | 雲端向量庫、Google Drive→向量→Pinecone、Gemini Embeddings、Chat + 檢索 | [04_Pinecone雲端向量庫](./04_Pinecone雲端向量庫/README.md) |
| 5 | 拓展 | 多模態 RAG 或企業應用 | 圖片/表格檢索、權限與多租戶、評測與優化 | [05_多模態與企業應用](./05_多模態與企業應用/README.md) |
...
- **進階**：[04_Pinecone雲端向量庫](./04_Pinecone雲端向量庫/README.md)
...
│          ↓                                       │
│                                                  │
│  🟡 階段二：基礎版（Supabase 雲端儲存）          │
│     難度：⭐⭐☆                                   │
│     目標：掌握實用的 RAG 系統設計                │
│                                                  │
│          ↓                                       │
│                                                  │
│  🔴 階段三：進階版（本地 PostgreSQL）            │
│     難度：⭐⭐⭐                                  │
│     目標：建立企業級 RAG 應用                    │
│                                                  │
└──────────────────────────────────────────────────┘
...
| 特性 | 🟢 入門版 | 🟡 基礎版 | 🔴 進階版 |
|------|----------|----------|----------|
| **儲存方式** | In-Memory | Supabase（雲端） | PostgreSQL（本地） |
| **資料持久性** | ❌ 執行後消失 | ✅ 雲端儲存 | ✅ 本地持久化 |
| **設定複雜度** | 🟢 零設定 | 🟢 簡單 | 🟡 中等 |
| **技術門檻** | 🟢 零門檻 | 🟢 零門檻 | 🟡 需要終端機 |
| **工作流程數** | 1 個（整合式） | 4 個（分離式） | 4 個（分離式） |
| **文件來源** | 單一上傳 | 多來源整合 | 多來源整合 |
| **適用規模** | <50 文件 | <1000 文件（免費額度） | 無限制 |
| **查詢效能** | 🐢 較慢 | 🚀 快 | 🚀 最快 |
| **管理介面** | ❌ 無 | ✅ 視覺化儀表板 | ❌ 終端機 |
| **資料控制** | ✅ 本地 | ⚠️ 雲端 | ✅ 100% 本地 |
| **備份** | ❌ 無 | ✅ 自動備份 | ❌ 需手動設定 |
| **成本** | 🆓 免費 | 🆓 免費額度<br>💰 超過需付費 | 🆓 完全免費 |
| **適合場景** | 學習、測試 | 快速部署、小型專案 | 企業級、完全控制 |
...
### [📂 前往基礎版教學](./02_基礎版_Supabase雲端儲存/README.md)
...
**💡 適合**：想要快速開始，不想處理技術細節的使用者

---

## 🚀 階段三：進階版 - 本地 PostgreSQL ⭐⭐⭐

### [📂 前往進階版教學](./03_進階版_本地PostgreSQL/README.md)

**🎯 學習目標**：建立完全掌控的企業級 RAG 系統

#### **核心特色**

✅ **完全本地**：100% 資料控制，不依賴雲端服務  
✅ **零成本**：PostgreSQL + PGVector 完全免費開源  
✅ **無限制**：沒有資料量或請求數限制  
✅ **高效能**：本地查詢，延遲最低  
✅ **企業級**：適合需要完全掌控的應用  
✅ **可擴展**：支援大規模資料和進階功能
...
| **基礎版** | Google Gemini（免費） | Supabase（免費額度） | Google Gemini（免費） | **$0-25** |
| **進階版** | Google Gemini（免費） | PostgreSQL（免費） | Google Gemini（免費） | **$0** |

**💡 提示**：所有版本都可以完全免費使用！基礎版超過免費額度才需付費。
...
### 🔴 [開始進階版 →](./04_Pinecone雲端向量庫/README.md)
**建立百萬級雲端向量知識庫**
...
[⬅️ 返回階段二：AI Agent 核心與工具調用](../階段二_AI_Agent核心與工具調用/README.md) ｜ [➡️ 前往階段四：企業級進階實戰與多代理](../階段四_企業級進階實戰與多代理/README.md) ｜ [🏠 返回 AI 總目錄](../README.md)
