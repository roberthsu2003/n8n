# 整合 LLM 模型的 AI Agent
## 範例 3：多工具整合即時天氣與新聞助理（多工具動態決策）

### 📚 工作流程說明

在掌握基本對話功能後，學習如何讓 AI Agent 具備**多工具自主決策能力**。AI Agent 能根據使用者提問，自主在「即時天氣查詢工具（Open-Meteo API）」與「新聞 RSS 訂閱工具」之間做選擇，並透過 **`$fromAI()`** 動態決定城市座標或新聞來源！
...
```mermaid
flowchart TD
    A["💬 用戶詢問「今天台北天氣如何？有什麼重大科技新聞？」"] --> B["🤖 AI Agent (決策大腦)"]
    C["🧠 語言模型 (Gemini / Ollama / OpenAI)"] --> B
    D["💾 Simple Memory (對話記憶)"] --> B
    
    B -->|"AI 判斷：需要查天氣"| E1["🌤️ 即時天氣查詢 (HTTP Request Tool)"]
    B -->|"AI 判斷：需要查新聞"| E2["📰 科技新聞讀取 (RSS Read Tool)"]
...
- [📥 天氣和新聞查詢工作流程樣版 (天氣和新聞查詢_使用Ollama.json)](./天氣和新聞查詢_使用Ollama.json)
...
### 前置範例
建議先完成：
- [智能客服聊天機器人](../智能客服聊天機器人/README.md) - 學習基本對話功能

### 後續範例建議
完成此範例後，建議繼續學習：

1. **RAG** — [RAG 智能問答系統](../RAG智能問答系統/README.md)（完整 RAG 由淺至深）；精簡版：[文件智能問答系統](../文件智能問答系統/README.md)（選讀）
   
2. [具備工具使用能力的助理](../具備工具使用能力的助理/README.md)
