# 整合 LLM 模型的 AI Agent
## 範例 1：智能客服聊天機器人（純對話與對話記憶）

> 💡 **教學友善版本**  
> 本範例支援 **本地 Ollama（零成本）**、**OpenRouter**、**Google Gemini** 與 **OpenAI**，可快速體驗對話記憶與 System Prompt 設定！

---

### 📚 工作流程說明

這個範例展示如何建立一個最基礎且完整的 AI 聊天機器人：
- 接收用戶自然語言對話訊息（Chat Trigger）
- 透過 System Prompt 定義專屬客服角色與回答規範
- 利用 Window Buffer Memory 保持多輪對話脈絡
- 提供客製化、親切且具專業度的客服回覆
...
```mermaid
flowchart LR
    A["💬 用戶發送對話訊息 (Chat Trigger)"] --> B["🤖 AI Agent 核心大腦"]
    C["🧠 語言模型 (Ollama / Gemini / OpenAI)"] --> B
...
- [📥 智能客服聊天機器人工作流程樣版 (智能客服聊天機器人.json)](./智能客服聊天機器人.json)
